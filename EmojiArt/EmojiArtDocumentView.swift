...
struct EmojiArtDocumentView: View {
    ...
    private func convertToEmojiCoordinates(_ location: CGPoint, in geometry: GeometryProxy) -> (x: Int, y: Int){
        ...
        return (
            Int((CGFloat(location.x) - panOffset.width - center.x) / zoomScale),
            Int((CGFloat(location.y) - panOffset.height - center.y) / zoomScale)
        )
    }
    ...
    private func panGesture() -> some Gesture {
        DragGesture()
            .updating($gesturePanOffset) { latestPanOffset, gesturePanOffset, _ in
